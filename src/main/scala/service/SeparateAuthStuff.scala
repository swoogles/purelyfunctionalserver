...
class SeparateAuthStuff[F[_]: ConcurrentEffect]()(implicit f: Sync[F]) extends Http4sDsl[IO] {
  ...
  def getAccessToken[F[_]](request: Request[F]): Option[AccessToken] =
    request.params.get("access_token")
  ...
  def extractAuthParametersAndTurnIntoTypedRequest[G[_]: ConcurrentEffect](request: Request[G]): Option[AuthorizedRequest[G]] =
  getAccessToken(request) match {
    case Some(accessToken) => getFederatedSubFromAccessToken(accessToken) match {
  ...
  def createAuthenticatedRequest[G[_]: ConcurrentEffect](
          request: Request[G], pf: PartialFunction[AuthorizedRequest[G], G[Response[G]]])
    : Request[G] => G[Response[G]]
  = {
    request: Request[G] => extractAuthParametersAndTurnIntoTypedRequest(request) match {
