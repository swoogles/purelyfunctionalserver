...
//    def shortestPath(startingPoint: String, endingPoint: String, pathSoFar: List[String]): Map[String, List[String]] = {
    if (startingPoint == endingPoint) {
      if(currentPath.length < shortestPathsSoFar.get(endingPoint).map(_.length).getOrElse(Int.MaxValue)) {
      ...
      if (shortestPathsSoFar.contains(startingPoint)) {
        ...
        nodes.get(startingPoint) match {
          case Some(neighbors) =>
            neighbors.map(neighbor => shortestPaths(neighbor, endingPoint, shortestPathsSoFar.updated(startingPoint, currentPath :+ startingPoint), currentPath :+ startingPoint))
              .foldLeft(shortestPathsSoFar){ case (map1, map2) => map1.merged(map2){ case ((key1, value1), (key2, value2)) => (key1, if (value1.length < value2.length) value1 else value2)}}
          case None => HashMap()
        }
